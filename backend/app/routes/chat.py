...
async def chat_stream(body: ChatIn, user: User = Depends(check_rate),
    ...
    _SB243_DISCLOSURE = (
        "This is an AI assistant, not a human. Responses are generated by artificial "
        "intelligence and should be reviewed before sharing with customers."
    )

    async def event_gen():
        yield f"data: {json.dumps({'type': 'meta', 'conversation_id': conv.id, 'ai_disclosure': _SB243_DISCLOSURE})}\n\n"
        errors = []
        for name in chain:
            try:
